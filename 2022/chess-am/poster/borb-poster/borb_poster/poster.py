...
def _header(
    ...
    pdf.Heading(
        "Eirik Rolland Enger*, Audun Theodorsen, Rune Graversen",
        font="Helvetica-Bold",
        font_color=pdf.HexColor(color_palette.MAIN_COLOR),
        font_size=Decimal(25),
...
def create_paragraphs(layout: PageLayout) -> None:
    ...
    _caption_previous_object(
        ...
        as input to an NCL script, also present in volcano-cooking, which further
        generates the full forcing file. Raw emission data shown in the image titled
        ...
        files with a format that CESM2 accepts is created. The images in the box "CESM2
        output" are made from data generated by CESM2 using the forcing file "Synthetic
        data". "Response" show the shortwave minus longwave forcing in the lower left
        part and the temperature response in the upper right part, while "Aerosol 2D"
        shows the aerosol optical depth on the day of the eruption and six months after.
        You can view the full animation of the aerosol evolution by following the QR
        code in the bottom right of the figure!
        """,
        layout,
        location="left",
    ...
    _caption_previous_object(
        ...
        three month delay of the eruption, placing the eruptions in the ensembles in all
        four seasons. This seasonal variability is also what causes the structure seen
        in the percentiles. The vertical line indicate the time of the eruption.
        """,
        layout,
        location="bottom",
        y_shift=Decimal(43),
    ...
    layout.add(
        ...
        .add(
            _paragraph_text(
                ...
                for the temperature response to volcanoes, shown in the equation below.
                It is a convolution equation, where forcing is convolved with a general
                shape representing a response function, and a general duration time for
                the temperature response to volcanic eruptions is assumed.
                """,
                no_margins=True,
            )
        )
        ...
        .add(
            _paragraph_text(
                ...
                function by deconvolving temperature and forcing, provided we feed the
                algorithm with an initial guess of the response function [3, 4].
                """,
                no_margins=True,
            )
        )
